...
Explanations:
At each iteration of the algorithm, we take stock of some artifical attractive forces (final reference point) and repulsive forces (obstacles such as walls) acting on the robot. We calculate the direction of the resulting artificial force and multiply it by an amplitude equal to the distance to be traveled in one time step (distance = speed × time step).
